...
from .layers.residuelBlock import residuelBlock
from .layers.batchNormalization import batchNormalisation
import torch
import math
import time
import sys
...
class HE_CNN(torch.nn.Module):
    ...
    def calculate_depth(self):
        ...
        for layer_name, layer in self.model.named_children():
            ...
            if layer.__class__.__name__ == 'ResidualBlock':
                ...
                Out.h = (Out.h + 2 * layer_params.downsample[0].padding[0] - layer_params.downsample[0].kernel_size[0]) // layer_params.downsample[0].stride[0] + 1
                Out.w = (Out.w + 2 * layer_params.downsample[0].padding[1] - layer_params.downsample[0].kernel_size[1]) // layer_params.downsample[0].stride[1] + 1
    ...
    def forward(self, C_in: list, _time=False):
        ...
        for layer_name, layer in self.model.named_children():
            ...
            if layer.__class__.__name__ == 'BatchNorm2d':
                batchNormalisation(self.context, Out, self.Img, layer, self.data_size)

            if layer.__class__.__name__ == 'ResidualBlock':
                Out = residuelBlock(
                    self.context, Out, self.Img, layer, self.data_size
                )
            if layer.__class__.__name__ == 'Conv2d':
